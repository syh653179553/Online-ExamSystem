     ...
     <div class="flexarea">
       ...
       <transition name="slider-fade">
         <div class="right box-shadow">
           <div class="top">
             <ul class="item box-shadow">
               <li></li>
               <li class="exam-name">{{examData.type}}-{{examData.source}}</li>
               <li ref="toggle" @click="slider_flag = !slider_flag">
                 <i v-if="slider_flag" class="iconfont el-icon-d-arrow-right icon20"></i>
                 <i v-if="!slider_flag" class="iconfont el-icon-d-arrow-left icon20"></i>
               </li>
             </ul>
           </div>
...
<!--           </div>-->
           <div class="content box-shadow">
             <p class="topic">
               <span class="number">第{{number}}题(<span v-if="currentType == 1" class="font-color">选择题</span>
                 <span v-if="currentType == 2" class="font-color">填空题</span>
                 <span v-if="currentType == 3" class="font-color">判断题</span>)：</span>
               <br/>{{showQuestion}}</p>
             <div v-if="currentType == 1">
               ...
               <div class="analysis" v-if="isPractice && practiceSign">
                 <ul>
             ...
             <div class="sign">
               <i @click="practiceSign = !practiceSign" v-if="isPractice" class="icon-style el-icon-s-opportunity"
                  :class="{'lightsign': practiceSign == true}">
                 <span>解析</span>
               </i>
               <i @click="mark()" class="el-icon-s-flag"><span>标记</span></i>
             </div>
           </div>
           <div class="operation box-shadow">
             <ul class="end">
               <el-row class="chang-question">
                 <el-col :span="12"><div class="grid-content bg-purple">
                   <li @click="previous()"><i class="iconfont el-icon-back"></i><span>上一题</span></li>
                 </div></el-col>
                 <el-col :span="12"><div class="grid-content bg-purple-light">
                   <li @click="next()"><span>下一题</span><i class="iconfont el-icon-right"></i></li>
                 </div></el-col>
               </el-row>
        ...
        <!--左边题目编号区-->
        <transition name="slider-fade">
          <div class="left box-shadow" v-if="slider_flag">
            <div class="box-shadow bgc1" style="margin-bottom: 20px;">
              <div class="modeTitle">
                <div class="setbg">
                  <span>背景颜色:</span>
                  <span class="bgcChoose box-shadow bgc1"></span>
                  <span class="bgcChoose box-shadow bgc2"></span>
                  <span class="bgcChoose box-shadow bgc3"></span>
                </div>
              </div>
            </div>
            <div class="count-down">
              <i class="el-icon-time"></i><span>倒计时：<b>{{time-1}}:{{second}}</b></span>
            </div>
            <h4 class="answerCard">答题卡</h4>
            <ul class="l-top">
              <li>
                <a href="javascript:;"></a>
                <span>当前</span>
            ...
            <div class="l-bottom">
              ...
              <div class="final" @click="commit()">我要交卷</div>
            </div>
          </div>
        </transition>
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios(`/api/exam/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
        ...
        this.answerScore = this.examData.totalScore
        ...
        this.$confirm("考试结束时间未到,是否提前交卷","友情提示",{
          ...
          this.$axios({
            ...
            if(res.data.code == 200) {
              let startTimeHour = this.startTime.split(" ")[1].split(":")[0];
              let startTimeMinute = this.startTime.split(" ")[1].split(":")[1];
              let endTimeHour = this.endTime.split(" ")[1].split(":")[0];
              let endTimeMinute = this.endTime.split(" ")[1].split(":")[1];
              let lastTime = (endTimeHour - startTimeHour) * 60 + (endTimeMinute - startTimeMinute);
              this.$router.push({path:'/studentScore',query: {
                examname: this.examData.source,
                examtype: this.examData.type,
                totalTime: this.time,
                score: finalScore,
                totalScore: this.answerScore,
...
.border {
  position: relative;
  border: 1px solid #2776DF !important;
}
.bg {
...
.bgc2{
  background-color: rgb(214, 226, 207);
}
.bgc3{
